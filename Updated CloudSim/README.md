...
3. Create and Start Storage Nodes
   ...
   bash# Terminal 2: Create and start Node 1
...
python start_node.py node2 4. Upload Files
In another terminal, upload a file:
bashpython upload_client.py yourfile.txt
🎮 Automated Demo
...
Create and start two storage nodes
Upload the test file automatically
Open everything in separate terminals

📁 Project Structure
distributed-cloud-storage/
├── cloud*gateway.py # Main gateway server
...
└── storage*\*/ # Node storage directories (created at runtime)
🔧 Components
Cloud Gateway
...
(Note: Basic implementation included)

💻 Usage Examples
Creating Nodes with Custom Configuration
bash# Node with 5GB storage and 4 CPU cores
...
List All Nodes
bashpython create_node.py --list
Upload Various File Types
bashpython upload_client.py document.pdf
python upload_client.py image.jpg
python upload_client.py video.mp4
Upload to Remote Gateway
bashpython upload_client.py file.txt remote_host 8000
🧪 Testing
...
Client uploads file to Gateway
Gateway splits file into chunks (default: 3 chunks)
Gateway selects available nodes using round-robin
Gateway sends chunks to selected nodes
Nodes store chunks on disk and send ACK
Gateway saves metadata to database
Client receives file ID and confirmation
